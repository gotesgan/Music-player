...
app.get('/search', async (req, res) => {
    ...
    try {
        ...
        if (!spotifyResult) return res.status(404).json({ error: 'Song not found on Spotify' });
...
app.get('/stream', async (req, res) => {
    ...
    if (!query && !id) return res.status(400).json({ error: 'Either "query" or "id" parameter is required' });

    try {
        let videoId = id;
        if (query && !id) {
            const spotifyResult = await searchSpotify(query);
            if (!spotifyResult) return res.status(404).json({ error: 'Song not found' });
        ...
        res.set({ 'Content-Type': 'audio/mpeg', 'Accept-Ranges': 'bytes', 'Cache-Control': 'no-cache' });
        ytdl(videoUrl, { filter: 'audioonly', quality: 'highestaudio' })
            .on('error', err => res.status(500).json({ error: 'Streaming failed', message: err.message }))
            .pipe(res);
    } catch (error) {
...
app.get('/info', async (req, res) => {
    ...
    if (!id) return res.status(400).json({ error: 'Video ID parameter "id" is required' });
...
app.get('/categories', async (req, res) => {
    try {
        ...
        console.error('Spotify categories error:', err.response?.data || err.message);
...
app.get('/categories/:id/playlists', async (req, res) => {
    ...
    try {
        ...
        console.log(`Spotify category ${req.params.id} playlists response:`, response.data);
        const playlists = (response.data.playlists?.items || response.data.items || []).map(p => ({
            id: p.id,
            name: p.name,
        ...
        console.error(`Spotify category ${req.params.id} playlists error:`, err.response?.data || err.message);
        res.status(err.response?.status || 500).json({
...
app.get('/genres', async (req, res) => {
    try {
        ...
        const staticGenres = [
            ...
            'rap', 'blues', 'indie', 'metal', 'folk', 'r&b', 'soul', 'reggae'
        ];
...
app.get('/recommendations', async (req, res) => {
    ...
    try {
        ...
        if (!tracks.length) {
            return res.status(404).json({ error: 'No tracks found for this genre or language' });
        }
